...
DURUM = {"d":"hazir", "muzik":False, "muzik_ad":"", "tema":"dark", "panel":None, "uyari":""}
...
_calan = {"p": None}
_ses_kilit = threading.Lock()   # ayni anda tek konusma
...
def muzik_cal(arama):
    def _iste():
        try:
            link = subprocess.run(["yt-dlp","-f","bestaudio","-g","--no-playlist",
            ...
            _muzik["ad"] = arama
            with _kilit: DURUM["muzik"] = True; DURUM["muzik_ad"] = (baslik or arama)[:60]
        except Exception as e:
...
def muzik_durdur():
    ...
    _muzik["p"] = None
    with _kilit: DURUM["muzik"] = False; DURUM["muzik_ad"] = ""
...
def konusma_dongusu(gecmis):
    ...
    with sd.InputStream(samplerate=SR, channels=1, dtype="float32", device=cihaz,
        ...
        while True:
            ...
            dusuk = metin.lower()
            wake = any(w in dusuk for w in WAKE)
