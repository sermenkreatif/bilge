...
def _simdi_str():
    g=["Pazartesi","Sali","Carsamba","Persembe","Cuma","Cumartesi","Pazar"]
    n=datetime.now()
    return n.strftime("%d.%m.%Y")+" "+g[n.weekday()]+", saat "+n.strftime("%H:%M")
def log(*a):
...
# Bulut STT (Deepgram) — yerel Whisper YOK, Pi'de agir AI yuku yok, isinma yok
DEEPGRAM_KEY= os.environ.get("DEEPGRAM_API_KEY","")
DG_MODEL    = "nova-2"   # Deepgram modeli (Turkce destekli)
DG_DIL      = "tr"
    ...
    try:
        ...
        while True:
            y=client.messages.create(model="claude-sonnet-4-6",max_tokens=800,
                system=SISTEM+" Su anki tarih ve saat: "+_simdi_str()+". Saat veya tarih sorulursa dogrudan bu bilgiyi soyle; asla UTC deme, bilgisayarini kontrol et deme, gercek zamani biliyorsun. HIZ KURALI: Sesli cevabin KISA olsun - normalde 1-2 cumle, en fazla 3. Uzun bilgi/liste gerekiyorsa kisaca ozetle ve detayi [PANEL]...[/PANEL] icine koy, sesli uzun uzun sayma. COK ONEMLI: Bu cevabin tamami bastan sona Turkce olacak. Tek bir Ingilizce kelime bile kullanma.",
...
def _deepgram_yaz(audio_f32, sr=16000):
    # float32 sesi WAV'a cevir, Deepgram buluta yolla, Turkce yaziyi dondur (yerel AI yok)
    import io, wave, json as _j, urllib.request, numpy as np
...
def mikrofon_dongusu():
    ...
    if not DEEPGRAM_KEY:
        log("UYARI: DEEPGRAM_API_KEY yok - konusma tanima calismaz (bilge.env kontrol et).")
    ...
    threading.Thread(target=hatirlatma_kontrol,daemon=True).start()
    threading.Thread(target=guc_izle,daemon=True).start()
    konus("Merhaba, ben Bilge. Sana nasil yardimci olabilirim?")
    q=[]
    ...
    with sd.InputStream(samplerate=SR,channels=1,dtype="float32",device=cihaz,
        ...
        while True:
            try:
                audio=_dinle_cumle()
                DURUM["d"]="dusunuyor"
                metin=_deepgram_yaz(audio,SR)     # <-- BULUT STT (yerel Whisper yok)
            except Exception as e:
